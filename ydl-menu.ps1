# ydl-menu.ps1 — yt-dlp interactive menu (Windows, PS 5.1 & 7 compatible)
...
function Read-Choice($prompt, $choices) {
    ...
    do {
        $sel = Read-Host "Pilih [1..$($choices.Count)]"
...
Ensure-Tools   # tanpa tanda kurung

# 1) URL
$url = Read-Host "Tempel link (YouTube/Instagram/Twitter/X)"
...
# 4a) Lokasi output default ke Downloads\ydl (aman dari permissions)
...
# 6) (Opsional) cookies dari Chrome — uncomment bila perlu
# $dlArgs += @("--cookies-from-browser","chrome")
...
if ($ec -ne 0) { Write-Host "Ada error. Coba tambah --verbose untuk detail." -ForegroundColor Yellow }
